...
from front.adafruit.i2c import I2CDevice, ROUnaryStruct, RWBit, ROBit, UnaryStruct, RWBits
...
class MAX17048:
    ...
    @property
    def value(self) -> int:
        return int(self.cell_percent)

    @property
    def charge(self) -> float:
        return abs(round(self.charge_rate, 1))

    @property
    def charging(self) -> bool:
        """ Whether the battery is charging or discharging. """
        return self.charge_rate > 0

    @property
    def hours(self):
        """ Time (in hours) required to charge or time left before empty. """
        return round(abs(100-self.cell_percent) / self.charge_rate, 2)
