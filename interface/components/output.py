...
class Output(Action):
    data_off = None
    name_data: str = "Data"
    name_amplification: str = "Amplification"
    ...
    def __init__(self, *, initialize: bool = True, containers=None,
        ...
        # Properties
        self._amplification = min(int(amplification), 100) if amplification > 0 else 10
        self._amplification_fade = self._amplification
        self._data = data

        # Behaviour
        self._period = period if period >= 0 else 0
        self._duty = duty if 0 <= duty < self._period else 0
        self._expiration = expiration if expiration >= 0 else 0
        ...
        # Events
        if does_blink:
            self._is_blinking = asyncio.Event()
        if does_expire:
            self._is_expiring = asyncio.Event()
            self._is_not_activated = asyncio.Event()
            self.expired = asyncio.Event()
        if does_fade:
            self._is_fading = asyncio.Event()
            self._is_fade_in = False
        ...
        if getattr(self, attr) != value:
            if not_off(value):
                ...
                if turn_off is None:
                    ...
                    turn_off()
            self.logging(f"{name} set to {value}", "INFO")
            self._update()
    ...
    async def blinking(self):
        ...
        while True:
            ...
            await self.blink()
            self.logging(f"Blinking: {self._period}", "TRACE")
